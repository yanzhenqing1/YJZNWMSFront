<template>
	<view>
		...
		<view class="u-m-t-20">
			<u-cell-group>
				<u-cell-item v-for="(item, index) in list" :key="index" icon="list" :title="item.barCode" :value="item.toQty + ' / ' + item.downQty" :arrow="false"></u-cell-item>
			</u-cell-group>
		</view>
	</view>
</template>
...
export default {
	...
	onLoad(option) {
		...
		this.model.localCode = option.l;
		this.model.storeCode = this.vuex_user.warehouse;
	...
	methods: {
		initData() {
			if (this.model.mainCode == undefined || this.model.mainCode == '') {
				this.model.mainCode = this.vuex_user.warehouse;
			}
			this.request({
				url: 'WmsMoveDetail/details',
				data: {
					code: this.model.mainCode,
					l: this.model.localCode
				},
				success: res => {
					this.list = res.data;
				}
